...
- (void)invokeStorageTaskEvent:(NSNumber *)handle type:(StorageTaskEventType)type snapshot:(FIRStorageTaskSnapshot *)snapshot {
...
- (NSDictionary *)buildDictionaryFromTaskSnapshot:(FIRStorageTaskSnapshot *)snapshot {
    ...
    [dictionary setValue:@((long)([snapshot.progress completedUnitCount])) forKey:@"bytesTransferred"];
    ...
    if([snapshot metadata] != nil) {
        [dictionary setValue:[self buildDictionaryFromMetadata:snapshot.metadata] forKey:@"storageMetadata"];
    }
    return dictionary;
}
